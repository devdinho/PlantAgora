...
from plantagora.models import Garden, GardenAddress, Grower, BaseAddress, Signature, GardenBed
...
class GardenAddressAdmin(admin.ModelAdmin):
    list_display = ('street', 'number', 'zip_code', 'city', 'garden', 'status',)
    search_fields = ('id', 'street', 'number', 'zip_code')
    list_filter = ('city', 'garden')
    icon_name = 'home'
    autocomplete_fields = ('garden','city',)
    

class GrowerAdmin(admin.ModelAdmin):
    list_display = ('profile','document', 'documentType',)
    search_fields = ('profile','document',)
    autocomplete_fields = ('profile','address',)
    icon_name = 'person'


class BaseAddressAdmin(admin.ModelAdmin):
    list_display = ('id','street', 'number', 'zip_code', 'city',)
    list_display_links = ('id', 'street', 'number', 'zip_code', 'city',)
    search_fields = ('id', 'street', 'number', 'zip_code')
    list_filter = ('city', )
    icon_name = 'home'
    autocomplete_fields = ('city',)
